...
function eventListeners(){ 
    form.addEventListener("submit",addTodo); //Form submit (addTodo) 
    document.addEventListener("DOMContentLoaded",loadedAllTodosToUI); //DOMContentLoaded (loadedAll) 
    secondCardBody.addEventListener("click",deleteTodo); //cardBody click
    filter.addEventListener("keyup",filterTodos); //filter (filterTodos)
    clearButton.addEventListener("click",clearAllTodos); 

function addTodo(e){
    let todos = getTodosFromStorage();//info from storage
...
function addTodoToStorage(newTodo){ //todo ekleme 
    let todos=getTodosFromStorage();
    todos.push(newTodo); 
    localStorage.setItem("todos",JSON.stringify(todos));
}
